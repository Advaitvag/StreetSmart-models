...
read -ra MODELS <<<"${STREETSMART_MODELS:-yolo11s yolo26s yolo26m yolo11m}"
...
[[ -x "$PY" ]] || {
  notify critical "night sweep aborted" "venv python missing: $PY"
  exit 1
}
[[ -f "$DATA" ]] || {
  notify critical "night sweep aborted" "dataset yaml missing: $DATA"
  exit 1
}
